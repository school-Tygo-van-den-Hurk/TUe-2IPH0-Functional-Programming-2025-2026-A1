...
Below you find concepts you have not dealt with before. For instance, you need to derive your own [instances for a type class](https://en.wikibooks.org/wiki/Haskell/Classes_and_types), work with a [`Data.Map.Strict`](https://hackage-content.haskell.org/package/containers-0.6.7/docs/Data-Map-Strict.html) structure. Useful functions could be [foldrWithKey](https://hackage-content.haskell.org/package/containers-0.6.7/docs/Data-Map-Strict.html#v:foldrWithKey), [filter](https://hackage-content.haskell.org/package/containers-0.6.7/docs/Data-Map-Strict.html#v:filter), [sort](https://hackage-content.haskell.org/package/base-4.22.0.0/docs/Data-List.html#v:sort). Additionally [list comprehensions](https://wiki.haskell.org/List_comprehension) can be useful.
...
3. Define the function, `toExprF` and `fromExprF`. Using `toExprF` in combination with `parseExpr` you should be able to get `Fix (ExprF String Integer)` from a string.
